...
fn test_profit_target_reached() {
    ...
    assert!(profit_rule.verdict.is_target_hit(), "expected TargetHit, got {:?}", profit_rule.verdict);
    // The overall decision should be TargetHit (not Pass), since this is the
    // most-positive thing that happened on this evaluation.
    assert_eq!(result.decision.kind, propfirm::engine::decision::DecisionKind::TargetHit,
        "expected decision=TargetHit, got {:?}", result.decision.kind);
}
...
fn test_pipeline_end_to_end() {
    ...
    let result = pipeline.process(account.id, PipelineEvent::Tick { tick, broker_equity: account.equity, broker_balance: account.balance }).unwrap();
...
fn p0_1_static_max_drawdown_does_not_breach_when_above_initial_floor() {
    ...
    let max_dd = result.reports.iter().find(|r| r.rule_name == "Maximum Drawdown").expect("max dd rule");
    assert!(max_dd.verdict.is_pass() || !max_dd.verdict.is_terminating(),
        "static max dd should not breach — account is above the floor; got {:?}", max_dd.verdict);
}
...
fn p0_2_target_reached_stays_pending_when_equity_dips_below() {
    ...
    let mut account = Account::new(AccountId::new(), plan).start(chrono::Utc::now() - chrono::Duration::days(2)).unwrap();
    ...
    // state, not breached).
    let profit_rule = result.reports.iter().find(|r| r.rule_name == "Profit Target").expect("profit rule");
    ...
    assert!(!result.decision.is_terminating(),
        "dip below target while pending is NOT a breach; got {:?}", result.decision.kind);
    // Verify the account's target_reached_at is still set (sticky).
    assert!(account.target_reached_at.is_some(),
...
fn p0_3_breach_beats_target_hit_on_same_tick() {
    ...
    assert!(result.decision.is_terminating(),
        ...
        result.decision.kind);
    assert_ne!(result.decision.kind, propfirm::engine::decision::DecisionKind::TargetHit,
...
fn p0_4_reordering_rules_produces_same_decision() {
    ...
    assert!(decision_a.is_terminating() && decision_b.is_terminating(),
        "both orders must produce a terminating decision");
    assert_eq!(decision_a.winning_priority, decision_b.winning_priority,
...
fn p1_5_estimated_equity_does_not_terminate_account() {
    ...
    let result = evaluator.evaluate_tick_estimated(&account, &tick, &[], &[], Vec::new()).unwrap();
    assert!(!result.decision.is_terminating(),
        "P1-5: estimated equity must NOT terminate; got {:?}", result.decision.kind);
    // But the same scenario with broker-reported equity SHOULD terminate.
    let result = evaluator.evaluate_tick(&account, &tick, &[], &[], Vec::new()).unwrap();
    assert!(result.decision.is_terminating(),
        "P1-5: broker-reported equity SHOULD terminate on a real breach; got {:?}", result.decision.kind);
}
...
fn p1_5_broker_reported_equity_terminates_on_real_breach() {
    ...
    let result = evaluator.evaluate_tick(&account, &tick, &[], &[], Vec::new()).unwrap();
    assert!(result.decision.is_terminating(),
        "broker-reported breach must terminate; got {:?}", result.decision.kind);
}
...
fn p1_14_stale_tick_is_rejected_by_pipeline() {
    ...
    let mut pipeline = propfirm::engine::pipeline::Pipeline::new(
        evaluator, store, propfirm::notifications::log::LogNotifier::new());
    ...
    let tick = Tick::new(Symbol::new("EURUSD"),
        Quote { bid: Price(dec!(1.0800)), ask: Price(dec!(1.0802)), ts: stale_ts });
    let result = pipeline.process(account.id, PipelineEvent::Tick {
        tick, broker_equity: account.equity, broker_balance: account.balance,
    });
    assert!(matches!(result, Err(propfirm::Error::TickRejected(_))),
        "stale tick must be rejected with TickRejected; got {:?}", result);
}
...
fn p1_14_out_of_order_tick_is_rejected_by_pipeline() {
    ...
    let tick1 = Tick::new(Symbol::new("EURUSD"),
        Quote { bid: Price(dec!(1.0800)), ask: Price(dec!(1.0802)), ts: t1 });
    let _ = pipeline.process(account.id, PipelineEvent::Tick {
        tick: tick1, broker_equity: account.equity, broker_balance: account.balance,
    }).unwrap();
    // Second tick at T-1 (older) — must be rejected.
    let t2 = t1 - chrono::Duration::minutes(1);
    let tick2 = Tick::new(Symbol::new("EURUSD"),
        Quote { bid: Price(dec!(1.0800)), ask: Price(dec!(1.0802)), ts: t2 });
    let result = pipeline.process(account.id, PipelineEvent::Tick {
        tick: tick2, broker_equity: account.equity, broker_balance: account.balance,
    });
    assert!(matches!(result, Err(propfirm::Error::TickRejected(_))),
        "out-of-order tick must be rejected; got {:?}", result);
}
...
fn p1_8_optimistic_concurrency_rejects_stale_write() {
    ...
    assert!(matches!(result, Err(propfirm::Error::StateConflict(_, _, _))),
        "stale write must be rejected with StateConflict; got {:?}", result);
}
...
fn p1_9_tenant_isolation_filter() {
    ...
    assert!(result.is_none(),
        "P1-9: tenant B must not see tenant A's account; got {:?}", result);
...
fn p1_11_override_clears_breach_state() {
    ...
    let _v = Violation::new(account.id, propfirm::core::ids::RuleId::named("max_drawdown"),
        "Maximum Drawdown", propfirm::core::violation::ViolationKind::MaxDrawdown,
        propfirm::core::violation::ViolationSeverity::Liquidate,
        "breach", chrono::Utc::now());
    let override_record = Override::new(account.id, violation_id,
        "Broker glitch tick on 2026-09-15 14:23 UTC; ticket #4521", "ops-alice",
        chrono::Utc::now());
    let state = propfirm::engine::state::AccountState::new(account);
    let new_state = state.clear_breach(&override_record).unwrap();
    assert_eq!(new_state.account.status, propfirm::core::account::AccountStatus::Active,
...
fn p1_12_emergency_stop_short_circuits() {
    ...
    let result = pipeline.process(account.id, PipelineEvent::EmergencyStop {
        reason: "Broker feed corrupted — freezing all accounts".into(),
        actor_id: "ops-bob".into(),
        at: chrono::Utc::now(),
    }).unwrap();
    assert_eq!(result.snapshot.account.status, propfirm::core::account::AccountStatus::EmergencyStopped,
...
fn p1_7_pure_evaluate_produces_stable_input_hash() {
    ...
    let mut plan = ftmo_phase1();
    let account1 = Account::new(AccountId::new(), plan.clone()).start(chrono::Utc::now()).unwrap();
    let account2 = Account::new(AccountId::new(), plan).start(chrono::Utc::now()).unwrap();
    let pack = RulePack {
        id: "test-pack-v1".into(), version: 1,
