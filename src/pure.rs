...
/// - `tick_or_event`: the triggering event (tick, order, trade fill).
...
/// triple, it always produces the same verdict and the same
/// `input_hash`. That's the property the binding spec calls
/// "one defensible answer" — and the property that enables EVL-33
/// replay/backfill.
pub fn evaluate(
    account: &Account,
    pack: &RulePack,
    ...
    //    same (account, pack, ctx_kind, open_positions, today_trades,
    //    pending_order, latest_trade, latest_tick) tuple always hashes
    //    to the same value, and always produces the same decision.
    let input_hash = compute_input_hash(account, pack, ctx_kind, open_positions, today_trades, pending_order, latest_trade, latest_tick);
...
/// Computes the `input_hash` (P1-7 fix). This is the cryptographic
/// fingerprint of every input that affects the verdict — so the same
/// inputs always produce the same verdict and the same hash.
///
/// Implementation note: uses the std default hasher (stable within a
/// process but not actually sha256). For real cross-process
/// reproducibility, swap in the `sha2` crate before going live —
/// the API of this function won't change.
pub fn compute_input_hash(
    ...
    // Hash the rule pack's content hash (already computed by the pack).
