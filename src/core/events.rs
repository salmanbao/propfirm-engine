...
use crate::core::ids::{AccountId, EventId, OrderId, PositionId, TradeId};
...
pub enum DomainEventKind {
    ...
    AccountStatusChanged { from: AccountStatus, to: AccountStatus },
    /// Account snapshot taken (e.g. on tick evaluation).
    AccountSnapshotted { snapshot: AccountSnapshot },
    /// Order lifecycle event.
    OrderEvent { order_id: OrderId, new_status: OrderStatus },
    /// Trade fill recorded.
    TradeFilled { trade: Trade },
    /// Position opened.
    PositionOpened { position_id: PositionId, symbol: Symbol, side: PositionSide, qty: crate::core::types::Quantity },
    /// Position closed.
    PositionClosed { position_id: PositionId, realized_pnl: Money },
    /// Tick received and equity updated.
    TickEvaluated { equity: Money },
    /// New trading day rolled over.
    DayRollover { new_day_index: u32, day_start: Money },
...
impl DomainEvent {
    pub fn new(account_id: AccountId, kind: DomainEventKind, occurred_at: Timestamp) -> Self {
        DomainEvent { id: EventId::new(), account_id, kind, occurred_at, causation_id: None }
    }
